...
final class CheckoutStepController
{
    ...
    public function list(Request $request): Response
    {
        ...
        // What gives a module's step a line on this screen without a migration of its
        // own. Creates the missing rows only, so nothing a merchant configured is lost.
    ...
    /**
     * The wording the merchant reads, in the language the back office is in, falling
     * back to the shop language and to the code last — a step a module just declared
     * has no wording at all until someone writes one.
     */
    private function title(CheckoutStep $step, string $locale): string
    {
        foreach ([$locale, $this->defaultLocale()] as $candidate) {
            $step->setLocale($candidate);
            $title = (string) $step->getTitle();
